/*
 * This file is part of KuberixEnterprise Console.
 * Copyright (C) 2019-2022 The KuberixEnterprise Console Authors.
 *
 * KuberixEnterprise Console is free software:                                                                    you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * KuberixEnterprise Console is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with KuberixEnterprise Console.  If not, see <https://www.gnu.org/licenses/>.
 */
module.exports = {
  CODE_REPO_PL: '코드 저장소',
  CODE_REPO_LOW: '코드 저장소',
  CODE_REPO: '코드 저장소',
  CODE_REPO_DESC: 'Git 기반으로 코드 저장소를 관리합니다.',
  CODE_REPO_EMPTY_DESC: 'Github, GitLab 또는 Bitbucket에서 코드 저장소를 가져옵니다. Git을 사용하여 소스 코드를 직접 가져올 수도 있습니다.',
  IMPORT_CODE_REPO: '가져오기 코드 저장소',
  EDIT_CODE_REPO: '코드 저장소 편집'
};