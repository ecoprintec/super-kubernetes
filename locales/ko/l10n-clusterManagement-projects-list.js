...
module.exports = {
  ...
  // List
  EMPTY_WRAPPER: '{resource}을(를) 찾을 수 없음',
  TERMINATING: '종료 중',
  ACTIVE: '활성',
  'Active': '활성',
  // List > Assign Workspace
  PROJECT_ADMINISTRATOR: '프로젝트 관리자',
  PROJECT_ADMINISTRATOR_DESC: '작업영역에서 사용자를 프로젝트 관리자로 선택하십시오.' ,
  PROJECT_ASSIGN_DESC: '프로젝트가 작업영역에 할당된 후에는 작업영역을 변경할 수 없습니다.' ,
  // List > Create
  CREATE_PROJECT_DESC: '자원을 그룹화하고 서로 다른 사용자의 자원 관리 권한을 제어하는 프로젝트를 만듭니다.' ,
  ...
  CREATE_NAME: '{name} 생성',
  DESCRIPTION: '설명',
  NAME_VALIDATION_FAILED: '이 이름은 쿠버네티스 시스템을 위해 예약된 쿠브-로 시작할 수 없습니다.',
  PROJECT_NAME_EXIST_DESC: '이름이 이미 존재합니다. 다른 이름을 입력하세요. 프로젝트 이름은 전체 플랫폼에서 고유해야 합니다.',
  NAME_EMPTY_DESC: '이름을 설정하십시오.' ,
  OK: 'OK',
  NAME_DESC: '이름은 소문자, 숫자 및 하이픈(-)만 포함할 수 있으며 소문자 또는 숫자로 시작하고 끝나야 합니다. 최대 길이는 63자입니다.' ,
  DESCRIPTION_DESC: '설명에는 임의의 문자가 포함될 수 있으며 최대 길이는 256자입니다.' ,
  ALIAS_DESC: '별명은 임의의 문자를 포함할 수 있으며 최대 길이는 63자입니다.' ,
  ...
  STOP_SUCCESS_DESC: '정상적으로 중지됨.' ,
  DELETE_RESOURCE_TYPE_DESC_SI: '{type} 이름 <strong>{resource}</strong>을 입력하여 이 작업의 위험을 이해하고 있는지 확인하십시오.' ,
  DELETE_RESOURCE_TYPE_DESC_PL: '이 작업의 위험을 이해하고 있는지 확인하려면 {type} 이름 <strong>{resource}</strong>을 입력하십시오.',
  DELETE_RESOURCE_TYPE_DESC_GW: '이 작업의 위험을 이해하고 있는지 확인하려면 {type} 이름 <strong>{resource}</strong>을 입력하십시오.'
};