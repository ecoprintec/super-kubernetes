...
Super Kubenetes binds a PVC to a PV that satisfies the request you set for the PVC, such as capacity and access mode. When you create an application workload, you can select the desired PVC and mount it to your workload.

1. Log in to the web console of Super Kubenetes as `project-regular` and go to a project. Click **Persistent Volume Claims** under **Storage** from the navigation bar, and you see all PVCs that have been mounted to workloads in the project.
...
4. On the **Storage Settings** page, select a method to create a PVC.
   ...
   - **From Storage Class**. You can configure storage classes both [before](../../../installing-on-linux/persistent-storage-configurations/understand-persistent-storage/) and [after](../../../cluster-administration/storageclass/) the installation of Super Kubenetes.
...
From the **More** drop-down menu, there are other additional options provided by Super Kubenetes based on the underlying storage plugin, also known as `Storage Capability`. PVC features include the following:
...
For more information about `Storage Capability`, see [Design Documentation](https://github.com/Super Kubenetes/community/blob/master/sig-storage/concepts-and-designs/storage-capability-interface.md).
...
Some in-tree or special CSI plugins may not be covered by `Storage Capability`. If Super Kubenetes does not display the correct features in your cluster, you can make adjustments according to [this guide](https://github.com/Super Kubenetes/Super Kubenetes/issues/2986).

### Monitor PVCs

Super Kubenetes retrieves metric data of PVCs with `Filesystem` mode from Kubelet to monitor PVCs including capacity usage and inode usage.

For more information about PVC monitoring, see [Research on Volume Monitoring](https://github.com/Super Kubenetes/Super Kubenetes/issues/2921).
...
2. Click <img src="/dist/assets/docs/v3.3/common-icons/three-dots.png" width="15" alt="icon" /> on the right of a PV, and you can perform the following:
