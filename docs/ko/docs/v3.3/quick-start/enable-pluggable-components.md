...
2. 이 파일에서 `enabled`에 대해 `false`를 `true`로 변경하여 설치하려는 플러그식 구성요소를 활성화하십시오. 다음은 참조용 [예시 파일]()<!--(https://github.com/Super Kubenetes/kubekey/blob/release-2.2/docs/config-example.md )-->입니다. 완료 후에는 파일을 저장합니다.
...
쿠버네티스에 Super Kubenetes를 설치할 때 아래와 같이 두 개의 YAML 파일을 적용하여 [ks-installer]()<!-- (https://github.com/Super Kubenetes/ks-installer/) -->를 사용해야 합니다.

1. 먼저 [cluster-configuration.yaml]()<!-- (https://github.com/Super Kubenetes/ks-installer/releases/download/v3.1.0/cluster-configuration.yaml) --> 파일을 다운로드하여 수정합니다.
